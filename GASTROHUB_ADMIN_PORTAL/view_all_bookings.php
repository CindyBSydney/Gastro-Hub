...
<body>
    <div class="container my-3">
        ...
        <div class="table-responsive">
            <table class="table table-sm table-hover table-dark table-bordered" style="border-radius:10px;">
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM tbl_borrowing_record WHERE approved = 0 ORDER BY record_id DESC LIMIT $offset, $itemsPerPage";
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            ?>
                            <td><a href="approve_booking.php?record_id=<?php echo($row["record_id"]); ?>&item_loan_type=<?php echo($item_result["item_loan_type"]); ?>" class="btn btn-outline-success">Approve</a></td>
                            <td><a href="reject_booking.php?record_id=<?php echo($row["record_id"]); ?>" class="btn btn-outline-danger">Reject</a></td>
                            <?php
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='15'>No pending bookings found.</td></tr>";
                    }
                    ?>
                    </tbody>
            </table>
        ...
        <div class="pagination">
            <?php
            // Pagination links
            $sql = "SELECT COUNT(*) AS total FROM tbl_borrowing_record WHERE approved = 0";
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();
            $totalItems = $row['total'];
            $totalPages = ceil($totalItems / $itemsPerPage);

            echo "<p>Showing <big style='color:#3C91E6;'>$itemsPerPage</big> bookings per page of <big style='color:#3C91E6;'>$totalItems</big> total bookings.</p>&emsp;";
